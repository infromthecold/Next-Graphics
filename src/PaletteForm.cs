...
namespace NextGraphics
{
	public partial class PaletteForm : Form
	{
		...
		private bool IsFourBitMode { get => Model.IsFourBitPaletteAutoBankingEnabled; }

		private int ValidatedTransparentIndex { get => IsFourBitMode ? Palette.TransparentIndex % 16 : Palette.TransparentIndex; }
		...
		private void CreateColourButtons()
		{
			...
			var buttonsPerAxis = 16;
			var minFrameSize = Math.Min(colourButtonsPanel.Width, colourButtonsPanel.Height);
			var buttonSize = minFrameSize / buttonsPerAxis;

			var x = 0;
			var y = 0;

			colourButtons = new Button[buttonsPerAxis * buttonsPerAxis];
			colourButtonBanks = new Button[buttonsPerAxis, buttonsPerAxis];
			...
			for (int c = 0; c < colourButtons.Length; c++)
			{
				...
				colourButtonBanks[y, x] = button;

				colourButtonsPanel.Controls.Add(colourButtons[c]);
